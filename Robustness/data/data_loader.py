...
def fgsm_attack(model, images, labels, loss_fn, epsilon=0.1, manual=False):
    """Generates adversarial examples using the Fast Gradient Sign Method (FGSM).

    This function perturbs the input images to maximize the model's loss, thereby generating adversarial examples.
    ...
    Returns:
        torch.Tensor: Adversarial examples generated by FGSM.
    """
...
def add_random_noise(images, num_pixels=10):
    """Change some pixels of the image to a fixed value.
    ...
    Returns:
        torch.Tensor: Noisy examples.
    """

    noisy_images = images.clone().detach()
    batch_size, _, height, width = images.shape
    for i in range(batch_size):
        for _ in range(num_pixels):
            h, w = np.random.randint(0, height), np.random.randint(0, width)
            noisy_images[i, :, h, w] = 1.0  # Whitening the pixel.
